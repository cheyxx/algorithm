...
multiple::algorithm(){
  ...
  for (j = 0 ; j < _multiplicand.length(); j++) {
      v_multiplicand.push_back(_multiplicand[_multiplicand.length() - j - 1] - '0');
  }
  ...
  for (i = 0; i < v_multiplier.size(); i++) {
      ...
      for (j = 0; j < v_multiplicand.size(); j++) {
          ...
          tmp.push_back(self);
          if (j == (v_multiplicand.size() - 1)  && (carry > 0) )
              tmp.push_back(carry);
          }
          if (0 == i) {
          _result = tmp;
            ...
            if (tmp.size() > _result.size()) {
              unsigned int i;
              for (i = 0; i < tmp.size() - _result.size(); i++)
                  _result.push_back(0);
            }
